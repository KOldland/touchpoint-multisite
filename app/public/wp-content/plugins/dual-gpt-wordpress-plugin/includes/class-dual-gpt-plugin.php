...
class Dual_GPT_Plugin {
    ...
    public function run_planner_author($request) {
        ...
        $initial_tier = $retry_compact_prompt ? 1 : 0;
        $prompt = '';
        $selected_tier = -1;
        $selected_size = 0;
        for ($tier = $initial_tier; $tier <= 2; $tier++) {
            $candidate_prompt = $this->build_author_prompt($article, $framework, $tier);
            $candidate_size = strlen($candidate_prompt);

            if ($tier === 0 && $candidate_size > $soft_prompt_limit) {
                error_log('[PLANNER][AUTHOR] Prompt approaching limit. Escalating tier for session ' . $session_id . ' article ' . $article_id . ' size=' . $candidate_size);
                continue;
            }

            if ($candidate_size > $hard_prompt_limit) {
                error_log('[PLANNER][AUTHOR] Prompt exceeds hard preflight threshold at tier ' . $tier . ' for session ' . $session_id . ' article ' . $article_id . ' size=' . $candidate_size);
                continue;
            }
        ...
        error_log('[PLANNER][AUTHOR] Using prompt tier ' . $selected_tier . ' for session ' . $session_id . ' article ' . $article_id . ' size=' . $selected_size);
        ...
        if (is_wp_error($job_id) && (string) $job_id->get_error_code() === 'prompt_too_long' && $selected_tier < 2) {
            $fallback_tier = $selected_tier + 1;
            $fallback_prompt = $this->build_author_prompt($article, $framework, $fallback_tier);
            if (strlen($fallback_prompt) <= $hard_prompt_limit) {
                error_log('[PLANNER][AUTHOR] Prompt too long after dispatch. Retrying fallback tier ' . $fallback_tier . ' for session ' . $session_id . ' article ' . $article_id);
                $job_id = $orchestrator->run_job($session_id, $idempotency . '-t' . $fallback_tier, $fallback_prompt, 'author');
                if (!is_wp_error($job_id)) {
                    $selected_tier = $fallback_tier;
                    $selected_size = strlen($fallback_prompt);
                }
            }
        }
        if (is_wp_error($job_id)) {
        ...
        foreach ($articles as $index => $item) {
            ...
            $articles[$index]['author'] = array(
                ...
                'post_id' => $post_id,
                'edit_url' => $edit_url,
                'prompt_size' => $selected_size,
                'compact_tier' => $selected_tier,
            );
            break;
        }
    ...
    private function build_author_prompt($article, $framework, $compact_tier = 0) {
        $compact_tier = max(0, intval($compact_tier));
        ...
        if ($compact_tier >= 2) {
            $framework = array(
                ...
                'key_themes' => isset($framework['key_themes']) && is_array($framework['key_themes']) ? array_slice($framework['key_themes'], 0, 4) : array(),
                'main_argument' => $framework['main_argument'] ?? '',
                'use_case' => $framework['use_case'] ?? '',
                'recommended_sections' => isset($framework['recommended_sections']) && is_array($framework['recommended_sections']) ? array_slice($framework['recommended_sections'], 0, 4) : array(),
            );
            $summary = mb_substr((string) $summary, 0, 280);
            $keywords = array_slice($keywords, 0, 4);
        } elseif ($compact_tier >= 1) {
            $summary = mb_substr((string) $summary, 0, 420);
        ...
        if ($compact_tier >= 2 && strlen($framework_json) > 1800) {
            $framework_json = substr($framework_json, 0, 1800) . '...';
        } elseif ($compact_tier >= 1 && strlen($framework_json) > 3000) {
            $framework_json = substr($framework_json, 0, 3000) . '...';
        }

        $citation_limit = $compact_tier >= 2 ? 2 : ($compact_tier >= 1 ? 3 : 5);
        $citation_json = wp_json_encode(array_slice($citations, 0, $citation_limit));
        ...
        $prompt = implode("\n", $lines);
        if ($compact_tier >= 2 && strlen($prompt) > 9200) {
            $prompt = substr($prompt, 0, 9200);
        } elseif ($compact_tier >= 1 && strlen($prompt) > 9600) {
            $prompt = substr($prompt, 0, 9600);
        }
    ...
    private function validate_research_phase_payload($phase_key, $payload, $meta = array()) {
        ...
        $blocked_terms = array('chatgpt', 'gemini', 'claude', 'ai-generated', 'synthetic study');
        $org_counts = array();
        $seen_keys = array();
        $type_counts = array(
            'academic' => 0,
        ...
        foreach ($citations as $index => $citation) {
            ...
            if ($url !== '') {
                ...
                foreach ($blocked_domains as $blocked_domain) {
                    if ($host !== '' && (substr($host, -strlen($blocked_domain)) === $blocked_domain)) {
                        $issues[] = $this->build_research_issue(
                            'error',
                            'blocked_domain',
                            $phase_key,
            ...
            if ($org !== '') {
                ...
                if ($org_counts[$org_key] > 2) {
                    $issues[] = $this->build_research_issue(
                        ...
                        $phase_key,
                        $path,
                        'More than 2 citations from the same organization.',
                        array('organization' => $org, 'count' => $org_counts[$org_key])
                    );
                }
            }
            ...
            if ($date_value === '' || $timestamp === null) {
                ...
                if ($timestamp < $cutoff) {
                    $issues[] = $this->build_research_issue(
                        ...
                        $phase_key,
                        $path . '.date',
                        'Citation is older than 36 months.',
                        array('value' => $date_value)
                    );
                }
            }
            ...
            foreach ($type_counts as $type => $count) {
                if ($count < 1) {
                    $issues[] = $this->build_research_issue(
                        ...
                        array('required_minimum' => 1, 'actual' => $count)
                    );
                }
            }
        }
