 ...
 *
 * Decomposes a WordPress post into atomic articles via LLMService (OpenRouter).
...
class AtomicArticleGenerator {
    ...
    public function generate( int $parent_id ) {
        ...
        if ( ! LLMService::is_configured() ) {
            return new \WP_Error( 'no_api_key', 'LLM API key not configured.', array( 'status' => 500 ) );
        }
        ...
        $units = $this->decompose_via_llm( $parent->post_title, $content );
    ...
    /**
     * Strip HTML and reduce post content to plain text suitable for a prompt.
     *
     * @param \WP_Post $post Post object.
     * @return string
     */
...
PROMPT;
        ...
        if ( JSON_ERROR_NONE !== json_last_error() || ! is_array( $parsed ) ) {
            error_log( '[KH Atomic] LLM returned invalid JSON: ' . substr( $raw, 0, 500 ) );
            return new \WP_Error( 'invalid_response', 'LLM returned unparseable JSON.', array( 'status' => 502 ) );
        }
    ...
    /**
     * Sanitize and validate each LLM-returned unit.
     *
