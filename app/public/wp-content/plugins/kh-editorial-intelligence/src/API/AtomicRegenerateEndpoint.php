 ...
 *
 * POST /wp-json/kh-editorial/v1/posts/{id}/atomic/regenerate
 *
 * Triggers synchronous LLM decomposition for a specific parent post.
...
use KH\Editorial\Services\GEO\AtomicArticleGenerator;
...
class AtomicRegenerateEndpoint {
    ...
    public function check_permission( \WP_REST_Request $request ) {
        ...
        if ( ! current_user_can( 'edit_post', $post_id ) ) {
            return new \WP_Error(
                'rest_forbidden',
                __( 'You are not allowed to regenerate atomic articles for this post.', 'kh-editorial-intelligence' ),
                array( 'status' => 403 )
            );
        }
    ...
    public function handle( \WP_REST_Request $request ) {
        ...
        if ( ! $post || AtomicArticlePostType::POST_TYPE === $post->post_type ) {
            return new \WP_Error(
                'invalid_post',
                __( 'Post not found or invalid post type.', 'kh-editorial-intelligence' ),
                array( 'status' => 404 )
            );
        }
        ...
        return rest_ensure_response( array(
            ...
            'message'  => sprintf(
                ...
                _n( '%d atomic article created.', '%d atomic articles created.', count( $result ), 'kh-editorial-intelligence' ),
                count( $result )
            ),
        ) );
    }
