 ...
 *
 * POST /wp-json/kh-editorial/v1/atomic/search
 *
 ...
 *   4. Calls LLM with the retrieved content to synthesise a grounded answer
 *   5. Returns {answer, sources[]} JSON
 *
 * Security:
 *   - Query capped at 500 chars
 *   - Per-IP rate limit via transients (10 requests / 60 s)
 *   - No user data stored; query is only sent to API
 *
 * @package KH\Editorial\API
 */
...
class AtomicSearchEndpoint {
    ...
    public function handle( \WP_REST_Request $request ) {
        ...
        if ( empty( $query ) ) {
            return new \WP_Error( 'empty_query', __( 'Query must not be empty.', 'kh-editorial-intelligence' ), array( 'status' => 400 ) );
        }

        if ( ! LLMService::is_configured() ) {
            return new \WP_Error( 'no_api_key', __( 'Search is not configured.', 'kh-editorial-intelligence' ), array( 'status' => 503 ) );
        }

        // 1. Embed the query.
        $query_embedding = $this->embedding_service->embed_now_text( $query );
        if ( is_wp_error( $query_embedding ) ) {
            return new \WP_Error( 'embed_failed', __( 'Failed to process query.', 'kh-editorial-intelligence' ), array( 'status' => 502 ) );
        }
        ...
        if ( empty( $all_embeddings ) ) {
            return rest_ensure_response( array(
                'answer'  => __( 'No content has been indexed yet. Please generate some atomic articles first.', 'kh-editorial-intelligence' ),
                'sources' => array(),
            ) );
        }
        ...
        if ( is_wp_error( $answer ) ) {
            return new \WP_Error( 'synthesis_failed', __( 'Failed to generate answer.', 'kh-editorial-intelligence' ), array( 'status' => 502 ) );
        }
    ...
    /**
     * Synthesise a grounded answer from retrieved context using LLM.
     *
...
Rules:
- Answer only from the provided articles. Do NOT использовать outside knowledge.
...
PROMPT;
        ...
        $route = LLMService::resolve_agent_model( 'atomic_article' );
        ...
        $result = LLMService::post_completion( $messages, array(
