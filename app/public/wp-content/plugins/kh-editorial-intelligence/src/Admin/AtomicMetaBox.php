 ...
 *  - "Regenerate Now" button (calls POST /kh-editorial/v1/posts/{id}/atomic/regenerate)
...
class AtomicMetaBox {
    ...
    public function add_meta_box(): void {
        ...
        foreach ( $post_types as $post_type ) {
            add_meta_box(
                'khm_atomic_articles',
                __( 'Atomic Articles', 'kh-editorial-intelligence' ),
    ...
    public function render_meta_box( \WP_Post $post ): void {
        ...
        <p>
            <label>
                <input
                    ...
                    <?php checked( $enabled ); ?>
                >
                <?php esc_html_e( 'Generate atomic articles on publish/update', 'kh-editorial-intelligence' ); ?>
            </label>
        </p>
        ...
        <p class="description">
            ...
            printf( esc_html( _n( '%d atomic article generated.', '%d atomic articles generated.', $count, 'kh-editorial-intelligence' ) ), $count );
            ?>
        </p>
        <p style="font-size:0.85em;">
            <a href="<?php echo esc_url( $filter_url ); ?>">
                <?php esc_html_e( 'View all in list →', 'kh-editorial-intelligence' ); ?>
            </a>
        </p>
        ...
        <p>
            ...
            >
                <?php $count > 0 ? esc_html_e( 'Regenerate Now', 'kh-editorial-intelligence' ) : esc_html_e( 'Generate Now', 'kh-editorial-intelligence' ); ?>
            </button>
    ...
    public function print_js(): void {
        ...
        (function () {
            ...
            btn.addEventListener('click', function () {
                ...
                status.textContent   = '<?php echo esc_js( __( 'Generating…', 'kh-editorial-intelligence' ) ); ?>';

                fetch('/wp-json/kh-editorial/v1/posts/' + postId + '/atomic/regenerate', {
                    method:  'POST',
                    headers: {
                ...
                .then(function (data) {
                    if (data.count !== undefined) {
                        status.textContent = data.count + ' <?php echo esc_js( __( 'atomic articles created. Reload to see them.', 'kh-editorial-intelligence' ) ); ?>';
                    } else {
                        status.textContent = data.message || '<?php echo esc_js( __( 'Done.', 'kh-editorial-intelligence' ) ); ?>';
                    }
                    btn.disabled = false;
                })
                .catch(function () {
                    status.textContent = '<?php echo esc_js( __( 'Error — check console.', 'kh-editorial-intelligence' ) ); ?>';
                    btn.disabled = false;
                });
            });
        }());
