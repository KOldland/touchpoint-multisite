 ...
 * Posts are generated automatically from parent WP posts via LLM decomposition.
 *
 ...
 *   _atomic_generated_at   string ISO 8601 timestamp of last LLM generation
 *
 ...
 *
 * @package KH\Editorial\PostTypes
 */

namespace KH\Editorial\PostTypes;
...
class AtomicArticlePostType {
    ...
    public function register_post_type(): void {
        register_post_type(
            self::POST_TYPE,
            array(
                'label'               => __( 'Atomic Articles', 'kh-editorial-intelligence' ),
                'labels'              => array(
                    'name'               => __( 'Atomic Articles', 'kh-editorial-intelligence' ),
                    'singular_name'      => __( 'Atomic Article', 'kh-editorial-intelligence' ),
                    'add_new_item'       => __( 'Add New Atomic Article', 'kh-editorial-intelligence' ),
                    'edit_item'          => __( 'Edit Atomic Article', 'kh-editorial-intelligence' ),
                    'view_item'          => __( 'View Atomic Article', 'kh-editorial-intelligence' ),
                    'search_items'       => __( 'Search Atomic Articles', 'kh-editorial-intelligence' ),
                    'not_found'          => __( 'No atomic articles found.', 'kh-editorial-intelligence' ),
                    'not_found_in_trash' => __( 'No atomic articles found in Trash.', 'kh-editorial-intelligence' ),
                ),
                'public'              => true,
    ...
    public function template_include( $template ): string {
        ...
        $plugin_template = KH_EDITORIAL_PLUGIN_DIR . 'templates/atomic-article.php';
    ...
    public function add_admin_columns( array $columns ): array {
        ...
        foreach ( $columns as $key => $label ) {
            $new[ $key ] = $label;
            if ( 'title' === $key ) {
                $new['parent_post'] = __( 'Parent Post', 'kh-editorial-intelligence' );
            }
        }
        return $new;
    }
    ...
    public function render_admin_column( string $column, int $post_id ): void {
        ...
        if ( ! $parent_id ) {
            echo '<em>' . esc_html__( '—', 'kh-editorial-intelligence' ) . '</em>';
            return;
        }

        $parent = get_post( $parent_id );
        if ( ! $parent ) {
            echo '<em>' . esc_html__( 'Deleted', 'kh-editorial-intelligence' ) . '</em>';
            return;
        }
        ...
        if ( $edit_url ) {
            printf(
                '<a href="%s">%s</a>',
                esc_url( $edit_url ),
                esc_html( $title ?: __( '(no title)', 'kh-editorial-intelligence' ) )
            );
        } else {
            echo esc_html( $title ?: __( '(no title)', 'kh-editorial-intelligence' ) );
        }
        ...
        printf(
            ...
            esc_url( $filter_url ),
            esc_attr__( 'Filter by this parent', 'kh-editorial-intelligence' )
        );
    }
