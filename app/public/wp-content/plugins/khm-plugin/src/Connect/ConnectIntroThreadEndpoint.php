...
class ConnectIntroThreadEndpoint {
	...
	public function __construct( ?ConnectIntroThreadRepository $threads = null, ?ConnectProviderRepository $providers = null ) {
	...
	public function create_thread( WP_REST_Request $request ) {
		...
		if ( '' === $buyer_name || '' === $buyer_email || ! is_email( $buyer_email ) || '' === $message ) {
			return new WP_Error( 'connect_intro_invalid_payload', __( 'Buyer name, email, and message are required.', 'khm-membership' ), array( 'status' => 400 ) );
		}
