...
/**
 * PAID-03 — LinkedInSandboxAdapter conformance tests.
 *
 * Verifies:
 ...
 */
final class LinkedInSandboxAdapterTest extends TestCase {

    private const FIXTURE = __DIR__ . '/../fixtures/golden/paid_adapter_dry_run_manifest.json';

    private LinkedInSandboxAdapter $adapter;
    ...
    protected function setUp(): void {
        ...
        $this->adapter = new LinkedInSandboxAdapter( null, $this->store );
    ...
    public function test_dry_run_has_required_fields(): void {
        ...
        $this->assertSame( 0.86, $op['confidence'] );
    }

    public function test_dry_run_is_deterministic(): void {
        // Stripping volatile timestamp before comparison.
    ...
    public function test_execute_returns_success(): void {
        ...
        $this->assertSame( 'linkedin_sandbox', $res['adapter_meta']['adapter'] );

        $op_result = $res['operation_results'][0];
        $this->assertSame( 'created', $op_result['result'] );
        $this->assertSame( 'li_op_51a139aa874b', $op_result['operation_id_on_channel'] );
        $this->assertSame( 59.35, $op_result['actual_spend'] );
    }
    ...
    public function test_execute_partial_failure_simulation(): void {
        ...
        $manifest['meta']['simulate_failures'] = [ 'op_1' => true ];
        ...
        $this->assertTrue( $op_result['error']['retryable'] );
                    ...
                    ->with(
                        ...
                        $this->callback( function ( $context ) {
                            return isset( $context['details']['adapter'] )
                                && 'linkedin_sandbox' === $context['details']['adapter']
                                && isset( $context['details']['manifest_id'] );
                        } )
                    );

        $adapter = new LinkedInSandboxAdapter( $mock_logger, $this->store );
