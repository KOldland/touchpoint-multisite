<?php
/**
 * LinkedIn Sandbox Adapter
 *
 * Deterministic, offline implementation of PaidAdapterContract for LinkedIn.
...
class LinkedInSandboxAdapter implements PaidAdapterContract {

    const ADAPTER_NAME    = 'linkedin_sandbox';
     ...
     * {@inheritdoc}
     *
     * Simulates LinkedIn Ads campaign creation deterministically.
     * Estimated spend = bid.amount × duration_days per operation.
     * Confidence is fixed at 0.86 for LinkedIn (accounts for targeting variance).
     */
    public function dry_run( array $manifest, array $opts = [] ): array {
        ...
        foreach ( $manifest['operations'] ?? [] as $op ) {
            ...
            $ops[] = [
                'operation_id'    => $op_id,
                'estimated_ops'   => [ 'create_campaign', 'create_creative', 'associate_audience' ],
    ...
    public function execute( array $manifest, array $opts = [] ): array {
        ...
        foreach ( $manifest['operations'] ?? [] as $op ) {
            ...
            $op_id_on_channel = DeterministicRng::operation_id( $seed, 'li_op' );
